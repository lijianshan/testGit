...
Page({
  ...
  onShow: function() {
    ...
    wx.getSetting({
      success: (res) => {
        console.log(res)
        that.setData({
          authorizationViewShow: !res.authSetting['scope.userLocation']
        })
        // 获取设备的地理位置(经纬度)
        if (that.data.authorizationViewShow == false) {
          wx.getLocation({
            type: 'wgs84',
            success: function(res) {
              console.log(res)
              // 请求获取室外空气质量
              that.sendGetRoomOutAirParameterStaterequest(res.latitude, res.longitude, true)
            }
          })
        }
      }
  ...
  AQIspecificationClk: function() {
    var that = this
    that.setData({
      AQIspecificationShow: that.data.AQIspecificationShow ? false : true
    })
  ...
  // 获取设备所在地室外空气状态
  sendGetRoomOutAirParameterStaterequest: function(lat, lon, isshow) {
    var that = this
    util.request({
      url: "/device/outdoors",
      data: {
